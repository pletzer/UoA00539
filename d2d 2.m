function [fd,pk,fnnb]=d2d(iv,d,opt,tt,tol2,slow)
%e.g. x=lorenz(); [fd,pk,fnnb]=d2d(x(3,:),3,0,1,10); or [fd,pk,fnnb]=d2d(x(3,:),3,0,1,10,1)
...
nnb=zeros(1,l2);
for i=1:l2
    nnb(i)=find(dists(:,i)-md(i)==0);
end
fnnb=0;
for i=1:l2
    if abs(iv(i+d)-iv(nnb(i)+d))/dists(i,nnb(i))>tol2
        fnnb=fnnb+1/l2;
    end
end
